...
export const useOrpData = () => {
  const [orpData, setOrpData] = React.useState(null);

  React.useEffect(() => {
    fetch("/orp.topo.json").then((response) => {
      if (response.ok) {
...
export const useKrajeData = () => {
  const [krajeData, setKrajeData] = React.useState(null);

  React.useEffect(() => {
    fetch("/kraje.topo.json").then((response) => {
      if (response.ok) {
        response.json().then((payload) => {
...
export const useCapacitiesData = () => {
  ...
  React.useEffect(() => {
    fetch("/capacities_data.json").then((response) => {
      if (response.ok) {
        response.json().then((payload) => {
